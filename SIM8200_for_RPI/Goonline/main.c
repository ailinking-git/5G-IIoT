...
#define CM_TIMESTAMP "2020-07-14 10:20:15"

static int utils_get_netdriver(const char *devname)
{
    int fd = -1;
    ...
    dbg_time("netcard driver = %s", drvinfo.driver);
	close(fd);
...
static char *
utils_get_driver (const char  *device_basename)
{
    static const char *subsystems[] = { "usbmisc", "usb" };
    unsigned int       i;
    char              *driver = NULL;
    char subdir[255];
    size_t last_nonslash,base,len;

    for (i = 0; !driver && i < sizeof(subsystems)/sizeof(subsystems[0]); i++) {
        char *path;
        ...
        /* device_basename + 4 equals cdc-wdm0 */
        snprintf(subdir, sizeof(subdir), "/sys/class/%s/%s/device/driver", subsystems[i], device_basename + 4);
        path = realpath (subdir, NULL);
        ...
        driver = malloc(len+1);
        memcpy (driver, path + base + 1, len);
        driver [len] = '\0';

        dbg_time("netcard driver = %s", driver);
    }

    if (!driver)
        dbg_time("[Warning]:Cannot get netcard driver, please check it.\n");
    return driver;
}
...
"-i, wwan interface               Specify network interface(default auto-detect\n"
...
"-6, Set ip-family                IPv4/IPv6 protocol\n"
...
static int qmidevice_detect(char **pp_qmichannel, char **pp_usbnet_adapter) {
    ...
    if (pDir)  {
        while ((ent = readdir(pDir)) != NULL)  {
            ...
            if (!strncasecmp(idVendor, "05c6", 4) || !strncasecmp(idVendor, "2c7c", 4) || !strncasecmp(idVendor, "1e0e", 4))
                ;
            else
                continue;
            ...
            snprintf(subdir, sizeof(subdir), "%s/%s:1.5/net", dir, ent->d_name);
            ...
            snprintf(subdir, sizeof(subdir), "%s/%s:1.5/GobiQMI", dir, ent->d_name);
            if (access(subdir, R_OK)) {
                snprintf(subdir, sizeof(subdir), "%s/%s:1.5/usbmisc", dir, ent->d_name);
                if (access(subdir, R_OK)) {
                    snprintf(subdir, sizeof(subdir), "%s/%s:1.5/usb", dir, ent->d_name);
                    if (access(subdir, R_OK)) {
...
#endif
{
    ...
    dbg_time("Build Version: %s", CM_TIMESTAMP);
...
__main_loop:
    ...
    // utils_get_netdriver(profile.usbnet_adapter);
    (void)utils_get_driver(profile.qmichannel);
