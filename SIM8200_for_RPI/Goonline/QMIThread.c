...
int qmiclientId[QMUX_TYPE_WDA + 1]; //GobiNet use fd to indicate client ID, so type of qmiclientId must be int
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    pRequest = ComposeQMUXMsg(QMUX_TYPE_WDA, QMIWDS_ADMIN_SET_DATA_FORMAT_REQ, WdaSetDataFormat, NULL);
...
__requestGetSIMStatus:
    ...
    pMUXMsg = &pResponse->MUXMsg;
    if (le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult) || le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError)) {
        dbg_time("%s QMUXResult = 0x%x, QMI_ERROR_CODE_TYPE = 0x%x", __func__,
            le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult), le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError));
...
struct wwan_data_class_str class2str[] = {
    ...
    // {WWAN_DATA_CLASS_NR5G_NSA, "NR5G_NSA"},
  ...
  /**<
    ...
    *pPSAttachedState = 0;
    ...
    if (DataCapList & WWAN_DATA_CLASS_LTE) {
        if (endc_available_valid && restrict_dcnr_valid) {
            if (endc_available && !restrict_dcnr) {
                DataCapList |= WWAN_DATA_CLASS_NR5G_NSA;
            }
        }
    }
#endif
    if (DeviceClass == DEVICE_CLASS_CDMA) {
    	...
    	if (DataCapList & WWAN_DATA_CLASS_NR5G) {
            pDataCapStr = wwan_data_class2str(WWAN_DATA_CLASS_NR5G);
		}
        // else if (DataCapList & WWAN_DATA_CLASS_NR5G_NSA) {
        //     pDataCapStr = wwan_data_class2str(WWAN_DATA_CLASS_NR5G_NSA);
        // }
...
__requestSetupDataCall:
    ...
    if (le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult) || le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError))
    {
        ...
        if (pTLVHdr) {
            ...
            dbg_time("Call End Reason is %d", call_end_reason);
        }
        ...
        if (pTLVHdr) {
            ...
            dbg_time("Call End Reason Type is %d", call_end_reason_type);
            dbg_time("Verbose Call End Reason is %d", verbose_call_end_reason);
        }
...
int requestBaseBandVersion(const char **pp_reversion) {
    ...
    if (revId && le16_to_cpu(revId->TLVLength))
    {
        ...
        if (s_9x07 == -1) { //fail to get QMUX_TYPE_WDA
